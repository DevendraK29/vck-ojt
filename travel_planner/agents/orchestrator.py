...
class OrchestratorAgent(BaseAgent[OrchestratorContext]):
    ...
    async def run(
        ...
        """
        self.logger.info(f"Running orchestrator agent with input: {input_data if isinstance(input_data, str) else '...'}")
    ...
    async def process(
        ...
        if context.planning_stage == PlanningStage.INITIAL:
            updated_requirements = await self._extract_requirements(input_data, context.travel_requirements)
        ...
        # Call the OpenAI API to get the orchestrator's response
