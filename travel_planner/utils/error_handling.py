...
def handle_errors(default_value: Optional[T] = None, error_cls: Type[Exception] = TravelPlannerError) -> Callable[[F], F]:
    """
    Decorator to catch and handle exceptions, logging them and optionally returning a default value.
    ...
    def decorator(func: F) -> F:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            try:
                ...
                raise error_cls(str(e), original_error=e)
...
def with_retry(
    ...
    """
    Decorator to retry a function with exponential backoff when specific exceptions occur.
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            try:
                ...
                if hasattr(last_attempt, "exception") and last_attempt.exception() is not None:
                    ...
                    func_name = func.__name__
                    logger.error(f"All retry attempts failed for {func_name}: {str(original_error)}")
...
def safe_execute(func: Callable[..., T], *args: Any, default: Optional[T] = None, **kwargs: Any) -> Optional[T]:
    """
    Execute a function safely, catching any exceptions and optionally returning a default value.
