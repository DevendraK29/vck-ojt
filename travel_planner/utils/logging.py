...
def setup_logging(log_level: Union[LogLevel, str] = LogLevel.INFO, log_file: Optional[str] = None):
    """
...
class AgentLogger:
    ...
    def __init__(self, agent_name: str, agent_id: Optional[str] = None):
        ...
        self.agent_id = agent_id or f"{agent_name}-{datetime.now().strftime('%Y%m%d%H%M%S')}"
    ...
    def log_api_request(self, api_name: str, endpoint: str, params: Optional[Dict[str, Any]] = None):
        """
        Log an API request.
    ...
    def log_api_response(self, api_name: str, endpoint: str, status_code: int, response_data: Optional[Any] = None):
        """
        Log an API response.
    ...
    def log_llm_input(self, model: str, messages: List[Dict[str, Any]], temperature: float):
        """
    ...
    def _safe_json(self, obj: Any) -> Optional[str]:
        """
        ...
        try:
            ...
            self.warning(f"Failed to serialize object to JSON: {str(e)}")
            return str(obj)